...
Note-⟩ If you get any type of bugs in this bot please inform our team at @cv_tf</b>");
$free = json_encode($keyboard);
        file_get_contents("https://api.telegram.org/bot<Bottoken>/editMessageText?chat_id=$cchatid2&text=$freecommands&message_id=$cmessage_id2&parse_mode=HTML&reply_markup=$free");
...
THIS BOT IS MADE WITH ♥️ BY  <code>@qqqqqqv</code> </b>",
 'parse_mode'=>'HTML',
 'reply_markup' => $free,
 ]);
        exit();
...
$premium_id = (array("1047661495","6472102283"));
$premiumgp = (array("-1001626972064"));
        $keyboard = json_encode([
'inline_keyboard' => [
[['text' => "OWNER", 'url' => "https://t.me/qqqqqqv"],]
]]);
                $keyboard1 = json_encode([
'inline_keyboard' => [
[['text' => "CHANNEL", 'url' => "https://t.me/sus_pablo"],
['text' => "GROUP", 'url' => "https://t.me/cv_tf"],]
]]);

// if(!in_array($chatId, $premiumgp)){
	// $tch = json_decode(file_get_contents("https://api.telegram.org/bot<Bottoken>/getChatMember?chat_id=@sus_pablo&user_id=".$gId))->result->status;
...
// exit();
// }
        	// $tch = json_decode(file_get_contents("https://api.telegram.org/bot<Bottoken>/getChatMember?chat_id=@sus_pablochats&user_id=".$gId))->result->status;
// if($tch != 'member' && $tch != 'creator' && $tch != 'administrator'){
// bot('sendMessage',['chat_id'=>$chatId,'reply_to_message_id'=>$message_id,'text'=>"Sorry <a href='tg://user?id=$gId'>$firstname</a>
// 
// You Need To Join My Update Channel To Get Regular Updates
// 
 // Made With ♥️ By @qqqqqqv And @sus_pablo
// 
// USE /start AFTER JOIN",
 // 'parse_mode'=>'HTML',
 // 'reply_markup' =>  $keyboard1,
...
edit_message($chatId,$message_id_1,$keyboard, "<b>? DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> EXPIRED KEY%0A%0A<b>Bot: @PaABLoooBot </b>");
}
elseif (strpos($result, 'Invalid API Key provided')){
edit_message($chatId,$message_id_1,$keyboard, "<b>═════════ 『 ROLDEX 』═════════%0A❌ DEAD KEY ❌</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> INVALID KEY%0A%0A<b>Bot: @PaABLoooBot </b>");
}
elseif ((strpos($result, 'testmode_charges_only')) || (strpos($result, 'test_mode_live_card'))){
edit_message($chatId,$message_id_1,$keyboard, "<b>═════════ 『 ROLDEX 』═════════%0A❌ DEAD KEY ❌</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> Testmode Charges Only%0A%0A<b>Bot: @PaABLoooBot </b>");
}else{
edit_message($chatId,$message_id_1,$keyboard, "<b>═════════ 『 ROLDEX 』═════════%0A✅LIVE KEY✅</b>%0A<u>KEY:</u> <code>${fii}xxxxxxxxxxxxxxxxx${newstring}</code>%0A<u>RESPONSE:</u> SK LIVE!!%0A%0A<b>Bot: @PaABLoooBot</b>");
// sleep(10);
...
global $botToken;
$url = "https://api.telegram.org/bot".$botToken."/editMessageText?chat_id=".$chatId."&message_id=".$message_id."&text=••MASS CHECKER%0A".$message."%0A••• BOT BY: @qqqqqqv&parse_mode=HTML";
...
<i>take group access from <code>@qqqqqqv</code> to use here</i></b>");
